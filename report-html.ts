import type {
  BatteryConfidence,
  BatteryDiagnosis,
} from '../battery-analysis/battery-analysis.types';
import type {
  BatteryReportItem,
  ReportQueryResult,
} from '../reports/report-query.types';
import { LOGO_DATA_URI } from './report-logo';
import { NOTE_ICON_DATA_URI } from './report-note-icon';
...
const STATUS_VIEW: Record<
  DashboardStatus,
  { className: 'ok' | 'warn' | 'crit'; label: string }
> = {
...
function deviceOrder(device: DashboardDevice): number {
  return { CRITICO: 0, ATENCAO: 1, OK: 2 }[device.status];
}
...
${device.motivo}`;
  ...
  return `<div class="cell ${statusView.className}${
    device.note ? ' has-note' : ''
        ...
        <span class="perf mono">${Math.round(device.perf)}<span style="font-size:9px">%</span></span>
      </div>`;
}
...
export function renderReportHtml(report: ReportQueryResult): string {
  ...
  const heatmapSplit = `<div class="heat-split">
        ...
        ${renderHeatmapArea('Monitoramento de pontos criticos', sensorDevices, 'sense')}
      </div>`;
...
<body>
  ...
  function order(d){ return {CRITICO:0,ATENCAO:1,OK:2}[d.status]; }
  ...
  const avgMinV = total ? DEVICES.reduce((a,d)=>a+d.minV,0)/total : 0;
  const lowConf = DEVICES.filter(d=>d.confidence==='BAIXA');
  ...
  /* ---------------- type bars ---------------- */
  (function(){
    const solar=DEVICES.filter(d=>d.tipo==='SOLAR'), fonte=DEVICES.filter(d=>d.tipo==='FONTE');
  ...
  /* ---------------- rank (worst) ---------------- */
  (function(){
    const worst=[...DEVICES].sort((a,b)=>a.perf-b.perf).slice(0,6);
